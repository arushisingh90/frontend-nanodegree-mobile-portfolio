...
var resizePizzas = function(size) {
  ...
  function changePizzaSizes(size) {
    ...
    //OPTIMIZATION: Moved dx and newwidth outside loop. Using element 1 since all elemts will have same size
...
// OPTIMIZATION: Moved pizzasDiv outside loop
...
function updatePositions() {
  ...
  var phaseArr = [];
  for (var j = 0; j < 5; j++){
      phaseArr.push(Math.sin(top + j) * 100);
  }

  for (var i = 0; i < window.itemsLen; i++) {
    window.items[i].style.left = window.items[i].basicLeft + phaseArr[i%5] + 'px';
  }
...
window.addEventListener('scroll', animateIfNotAnimating);

//OPTIMIZATION: Added requestAnimationFrame
...
document.addEventListener('DOMContentLoaded', function() {
  ...
  //OPTIMIZATION: Created global variables for items and item's length.
  //Didn't cause much chane in FPS but it shouldn't be fetched each time updatePositions is called
  window.items = document.getElementsByClassName('mover');
  window.itemsLen = window.items.length;
